...
<div class="page-container" style="margin-top: 50px">
    <form class="form-horizontal" id="product-edit" action="" method="post">
        ...
        <div class="form-group">`
            ...
            <div class="col-sm-6">
                <input type="text" class="form-control" id="name" value="${p.name}" name="name" placeholder="商品名称">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-6">
                ...
                <c:if test="${!empty p.subTitle}">
                    <input type="text" class="form-control" id="subTitle" value="${p.subTitle}" name="subTitle" placeholder="商品简述">
                </c:if>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <select class="form-control" id="cid" name="cid" placeholder="请选择商品分类">
                    <option value="${cs1.id}" selected>${cs1.name}</option>
                    <c:forEach items="${cs}" var="css">
                        <c:if test="${cs1.id ne css.id}">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control" id="originalPrice" value="${p.originalPrice}" name="originalPrice" placeholder="请输入正确的价格">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control" id="promotePrice" value="${p.promotePrice}" name="promotePrice" placeholder="请输入正确的价格">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control" id="stock" value="${p.stock}" name="stock" placeholder="0~9999">
            </div>
        </div>
        <div class="form-group">
            <input type="text" name="imageName" value="${p.imageName}" id="imageName" hidden/>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <c:if test="${empty p.detailed}">
                    <textarea name="detailed" style="width:100%;visibility:hidden;"></textarea>
                </c:if>
                <c:if test="${!empty p.detailed}">
                    <textarea name="detailed" style="width:100%;visibility:hidden;">${p.detailed}</textarea>
                </c:if>
            </div>
        </div>
        <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        //验证
        $("#product-edit").bootstrapValidator({
    ...
    $("#submitBtn").click(function () { //进行表单验证
        $("#product-edit").bootstrapValidator('validate');
        var bv = $("#product-edit").data('bootstrapValidator');
        bv.validate();
        if (bv.isValid()) {
            var index = layer.load(3);
            editor.sync();
//            console.log($("#product-edit").serialize());
            //发送ajax请求
            $.ajax({
                url: 'admin_product_edit?t=' + (new Date()).getTime(),
        ...
        editor = K.create('textarea[name="detailed"]', {
            ...
            afterCreate : function() {
                ...
                K.ctrl(document, 13, function() {
                    self.sync();
                    document.forms['product-edit'].submit();
                });
